...
class Unirgy_RapidFlowPro_Model_Mysql4_Eav extends Unirgy_RapidFlow_Model_Mysql4_Abstract_Fixed
{
    ...
    protected function _importRowEA($row)
    {
		...
		/*  ICUBE UPDATE : add available_for_vendor */ 
        $new = array( "attribute_code" => $row[1], "backend_type" => $row[2], "frontend_input" => $row[3], "frontend_label" => $this->_convertEncoding($row[4]), "is_required" => $row[5], "is_unique" => $row[6], "available_for_vendor" => $row[8] );
        $exists = $this->_write->fetchRow("select attribute_id,backend_type,frontend_input,frontend_label,is_required,is_unique,is_user_defined,available_for_vendor from " . $aTable . " where entity_type_id=" . $etId . " and attribute_code=?", $new["attribute_code"]);
        if( !$exists ) 
        {
    ...
    protected function _exportInitEA()
    {
        ...
        /*
	        ...
	        */
	    /*  ICUBE UPDATE : add available_for_vendor */ 
        $this->_select = $this->_read->select()->from(array( "main" => $attrTable ), array( "attribute_code", "backend_type", "frontend_label", "frontend_input", "is_required", "is_unique", "available_for_vendor" ))->join(array( "et" => $etTable ), "et.entity_type_id=main.entity_type_id", array( "entity_type" => "entity_type_code" ));
