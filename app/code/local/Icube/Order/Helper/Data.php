...
class Icube_Order_Helper_Data extends Mage_Core_Helper_Data
{
    ...
    {
    	Mage::log($data, false, "orderimport.log");
        ...
        foreach ($xml->order as $order) {
            ...
            $ordercreated[] = $orderObj->getIncrementId();
            ...
            if ($vendor->getId()) {
				...
				$order = Mage::getModel('sales/order')->load($orderObj->getIncrementId(), 'increment_id');
				$this->sendEmailOrderVendor($order, $vendor);

            }
        }

        echo 'Order created: '.implode(',', $ordercreated);
    }
    
    public function sendEmailOrderVendor($order, $vendor)
    {
	    $mailer = Mage::getModel('core/email_template_mailer');
        $emailInfo = Mage::getModel('core/email_info');
        $emailInfo->addTo($vendor->getEmail(), $vendor->getVendorName());
        $mailer->addEmailInfo($emailInfo);
        $mailer->setSender(Mage::getStoreConfig('sales_email/order_comment/identity', $order->getStore()->getId()));
