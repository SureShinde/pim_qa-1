...
class Peexl_ImportExportOrders_Model_Importexport extends Mage_Core_Model_Abstract
{
    ...
    public function _importOrders()
    {
        ...
        foreach ($xml->order as $order) {
            ...
            $store = $quote->getStore()->load($this->getStoreView());
    ...
    private function _getItemsStructuredArray($items)
    {
        ...
        foreach ($items as $item) {
            Mage::log(serialize($item->getProductOptions()), null, "options.log");
    ...
    public function _exportOrders(){
        $importReadyDir = Mage::getBaseDir('export');
        ...
        $creationTime = ($creationTime == 0) ? time() : $creationTime;

        if($this->getXls()=="yes"){
            //$exportFile = $importReadyDir . DS . 'ExportedOrders-' . $creationTime . "-" . $storeId .'.xls';
            $content    = Mage::app()->getLayout()->createBlock('importexportorders/adminhtml_sales_order_grid')->getExcelFile();

            $filename=end(explode('/',$content["value"]));

            return array('type'=>'xls',"file"=>$filename,'url'=>Mage::helper("adminhtml")->getUrl('*/*/download',array('f'=>$filename)));
        ...
        $count_orders = $this->countOrders($storeId);

        foreach ($orders as $order) {
            $xmlString .= $this->makeNew($order);
        }
        $xmlString = "<?xml version=\"1.0\" encoding=\"iso-8859-1\"?>\n<orders>\n" . $xmlString . "</orders>";


        $myFile = $importReadyDir . DS . 'ExportedOrders-' . $creationTime . "-" . $storeId . "-" . $stepNumber . '.xml';
        $fh = fopen($myFile, 'a') or die("can't open file");
        fwrite($fh, $xmlString);
        fclose($fh);

        $stepStop = 0;
        if($stepNumber == ceil($count_orders/$stepLength) || $count_orders == 0 ){
            $stepStop = 1;
        }
        $stepNumber += 1;

        $array = array('stepNumber' => $stepNumber, 'stepStop' => $stepStop, 'exported' => $orders->count(), 'time' => $creationTime,'type'=>'xml');
