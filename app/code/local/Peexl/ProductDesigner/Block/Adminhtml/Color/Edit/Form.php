...
class Peexl_ProductDesigner_Block_Adminhtml_Color_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        if (Mage::getSingleton('adminhtml/session')->getColorData())
        {
            $data = Mage::getSingleton('adminhtml/session')->getColorData();
            Mage::getSingleton('adminhtml/session')->getColorData(null);
        }
        elseif (Mage::registry('color_data'))
        {
            $data = Mage::registry('color_data')->getData();
        }
        else
        {
            $data = array();
        ...
        $fieldset = $form->addFieldset('color_form', array(
             'legend' =>Mage::helper('productdesigner')->__('Color Information')
        ));
